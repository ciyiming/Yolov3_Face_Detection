...
def create_modules(module_defs, regularization, loss, activation):
    """
    ...
    for module_i, module_def in enumerate(module_defs):
        ...
        if module_def["type"] == "convolutional":
            ...
            yolo_layer = YOLOLayer(anchors, num_classes, regularization, loss, img_size)
...
class YOLOLayer(nn.Module):
    ...
    def __init__(self, anchors, num_classes, regularization, loss, img_dim=416):
    ...
    def forward(self, x, targets=None, img_dim=None):
        ...
        p_boxes = FloatTensor(prediction[..., :4].shape)     # (b, 3, 13, 13, 4)
        p_boxes[..., 0] = x + self.grid_x
        p_boxes[..., 1] = y + self.grid_y
        p_boxes[..., 2] = torch.exp(w) * self.anchor_w
        p_boxes[..., 3] = torch.exp(h) * self.anchor_h
        ...
        if targets is None:
            ...
            #
            iou_scores, class_mask, obj_mask, noobj_mask, tx, ty, tw, th, tcls, tconf, target_boxes = build_targets(
            ...
            if self.loss == 0:
                reg_loss = loss_x + loss_y + loss_w + loss_h
            else:
                iou_loss = IoULoss()
                pred_boxes = torch.cat((x[..., None], y[..., None], w[..., None], h[..., None]), -1)[obj_mask]
                targ_boxes = torch.cat((tx[..., None], ty[..., None], tw[..., None], th[..., None]), -1)[obj_mask]
                #pred_boxes = p_boxes[obj_mask]
                #targ_boxes = target_boxes
                print('pred_boxes', pred_boxes.shape)
                print('targ_boxes', targ_boxes.shape)
                reg_loss = iou_loss(pred_boxes, targ_boxes, mode=self.loss, xyxy=False) * 4
            ...
            if self.regularization in [1, 3]:
                label_smooth_loss = LabelSmoothCELoss()
                loss_cls = label_smooth_loss(pred_cls[obj_mask], tcls[obj_mask], self.num_classes)

            # Total Loss
            total_loss = reg_loss + loss_conf + loss_cls
...
class Darknet(nn.Module):
    ...
    def __init__(self, opt, img_size=416):
        ...
        self.hyperparams, self.module_list = create_modules(self.module_defs, self.regularization, self.loss, self.activation)
        ...
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            if module_def["type"] in ["convolutional", "upsample", "maxpool"]:
                ...
                if self.training == True:
                    if self.regularization in [2, 3]:
                        dropblock = DropBlock2D(0.1, 7)
                        x = dropblock(x)
