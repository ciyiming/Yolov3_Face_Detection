...
def build_targets(pred_boxes, pred_cls, target, anchors, ignore_thres):
    ...
    gi, gj = gxy.long().t()     # .long()去除小数点，找到具体的cell
    # Set masks，这里的b是batch中的第几个
    # b代表那张图片，best_n代表那个anchor，gj, gi代表那个cell
    ...
    for i, anchor_ious in enumerate(ious.t()):
        ...
        # 因为每一组中的anchor_iou即使不是最大，也有可能很大，这样最终评价标准会产生歧义，所以需要设置一个thres，将anchor_iou大于thres的位置都不参与运算
        noobj_mask[b[i], anchor_ious > ignore_thres, gj[i], gi[i]] = 0

    # Coordinates
    tx[b, best_n, gj, gi] = gx - gx.floor()     # x_offset (0, 1)  # floor取整但不改变数据类型。上面的long也是取整，同时改变数据为长整形
    ...
    return iou_scores, class_mask, obj_mask, noobj_mask, tx, ty, tw, th, tcls, tconf, target_boxes
